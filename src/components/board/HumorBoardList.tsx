...
export function HumorBoardList(props: any) {
  ...
  useEffect(() => {
      axios.get(`http://localhost:8000/humor-board/?page=${page}`)
        .then(response => {
    ...
    return (
      ...
      >
        ...
        <Button alignSelf="flex-end" onClick={handleButtonClick}>
          <ChakraLink as={ReactRouterLink} to="/humor-board/writing">
            글쓰기
          </ChakraLink>
        </Button>
      </Flex>
        ...
        >
          ...
          <Tbody>
            {data.map((item) => (
              <Tr key={item.id}>
                <Td>
                  <ChakraLink as={ReactRouterLink} to={`/humor-board/detail/${item.id}`}>
                    <Text>
                      {item.title}{" "}
  ...
  return (
    <Container maxW={"5xl"}>
      <Box mt={150} mb={20}>
        ...
        <HumorBoardList title="유머"></HumorBoardList> 
